...
| `GUARDSCHOOL_PUBLIC_SCHOOL_HOST` | **Единый вход школ** (например `school.guarddoc.ru`): Host не мапится в slug поддомена; после `POST /api/login` по ключу лицензии в cookie задаётся школа (`gs_saas_tenant`), данные — из `tenants/<slug>/data`. Схема URL: `https://school.guarddoc.ru/` для всех; **код школы** при регистрации — только идентификатор в БД, не DNS. Опционально `GUARDSCHOOL_PUBLIC_SCHOOL_SCHEME` (по умолчанию `https`). Без этой переменной `school.guarddoc.ru` по-прежнему трактуется как тенант со slug `school` (старое поведение). |
...
| Лимиты (опционально) | `GUARDSCHOOL_SAAS_MAX_SCHEDULE_XLSX_BYTES` (по умолчанию 15 МБ), `GUARDSCHOOL_SAAS_MAX_SCHEDULE_JSON_BYTES` (1 МБ), `GUARDSCHOOL_SAAS_MAX_USER_DATA_BYTES` (15 МБ суммарно JSON), `GUARDSCHOOL_SAAS_MAX_WEEKLY_ZIP_BYTES` (15 МБ), `GUARDSCHOOL_MAX_WIDGET_IMAGE_UPLOAD_BYTES` / `GUARDSCHOOL_SCHOOL_NEWS_IMAGE_MAX_BYTES` (картинки админки). **413 при небольшом файле** — чаще nginx `client_max_body_size`: см. `cloud_guarddoc/nginx-guardschool-upload-limits.conf`. |
...
### Загрузки в админке (ошибка 413)

Если при загрузке картинок/файлов в кабинете школы появляется **413** при размере больше ~1 МиБ, на домене школы в nginx чаще всего действует лимит по умолчанию **1m**. Готовый фрагмент и команды `scp` / `include` — в **`cloud_guarddoc/nginx-guardschool-upload-limits.conf`** (скопируйте на сервер и подключите `include` в `server { }` для `school.*`).
