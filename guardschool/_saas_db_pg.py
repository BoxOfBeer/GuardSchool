...
def ensure_public_schema() -> None:
    ...
    with connect_public() as conn:
        with conn.cursor() as cur:
            _exec_many(
                ...
                [
                    ...
                    CREATE TABLE IF NOT EXISTS licenses (
                        ...
                        license_no BIGINT GENERATED BY DEFAULT AS IDENTITY,
            ...
            try:
                cur.execute(
                    "ALTER TABLE tv_access ADD COLUMN IF NOT EXISTS pin_bypass BOOLEAN NOT NULL DEFAULT FALSE"
                )
            except Exception:
                pass
            # Backward-compatible: license number and stored plaintext key (for admin recovery).
            try:
                cur.execute("ALTER TABLE licenses ADD COLUMN IF NOT EXISTS license_no BIGINT GENERATED BY DEFAULT AS IDENTITY")
            except Exception:
                pass
            try:
                cur.execute("ALTER TABLE licenses ADD COLUMN IF NOT EXISTS key_plaintext TEXT NOT NULL DEFAULT ''")
            except Exception:
                pass
            try:
                cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS licenses_license_no_uq ON licenses(license_no)")
            except Exception:
                pass
            # Backfill license_no for existing rows (identity will generate DEFAULT values).
            try:
                cur.execute("UPDATE licenses SET license_no = DEFAULT WHERE license_no IS NULL")
            except Exception:
                pass
...
def lookup_tenant_plan_id(slug: str) -> str | None:
    """plan_id активного тенанта из public.tenants (SaaS)."""
    if not saas_db_enabled():
        return None
    ts = (slug or "").strip().lower()
    if not ts:
        return None
    try:
        with connect_public() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT plan_id FROM tenants WHERE slug=%s AND status='active' LIMIT 1",
                    (ts,),
                )
                row = cur.fetchone()
                if row and row[0]:
