"""Оркестрация звонков и фона на переменах, tick_once, preview."""
...
from .local_audio_ffmpeg import _media_duration_sec
from .local_audio_playback import (
    AFTER_STOP_GRACE_SEC,
    _active_proc,
    _last_playback,
    _play_kind,
    _proc_alive,
...
def _orch_lesson_cleanup() -> None:
    ...
    if pk in _ORCH_KINDS:
        stop_playback_hard()
        time_module.sleep(AFTER_STOP_GRACE_SEC)
...
def get_break_music_playback_info() -> dict[str, Any]:
    ...
    if orch:
        ...
        out["mode_label"] = "Оркестрация перемен (расписание + файлы сигналов + фон)"
        ni = cursor % n
