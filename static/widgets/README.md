...
3. `tv-core-utils.js` → `tv-schedule-table.js` → `tv-screen-shell.js` — утилиты, таблица расписания, оболочка ТВ
4. `screen_widgets.js` — ядро, `helpers`, fallback для типов без плагина
5. `{type}.js` — рендер одного типа (после ядра)
